...
1. Before anything else, ensure BOTH GLM tools are loaded: if either
...
3. If the response says delegation is OFF (starts with "GLM delegation is OFF"),
   call `glm_set_mode(mode='on')` once, then retry step 2 a single time.
4. Return GLM's full response verbatim as your final message — including whichever of
...
5. If `delegate_to_glm` returns an `ERROR:` or not-configured message, return that
